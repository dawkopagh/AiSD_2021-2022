...
class NeighMatrix:
    ...
    def deleteEdge(self, vertex1, vertex2):
        ...
        self.neighList[self.getVertexIDx(vertex2)][self.getVertexIDx(vertex1)] -= 2 #metodę insert więc, a jako że jest to graf nieskierowany to nie do końca rozumiem takie działanie - dlatego dla potrzeby prezentacji, że krawędź faktycznie
...
for node in nodes:
    test.insertVertex(node)
for element in graf:
    if (element[1], element[0]) not in test.edges():
        test.insertEdge(element[0], element[1])

color = []
for inx, node in enumerate(nodes):
    color.append((node, inx))
print(color)

polska.draw_map(test.edges(), color)
